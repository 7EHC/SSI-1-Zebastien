...
const hasDataChanged = () => {
  ...
  newData.value.publishDate =
    `${pubShow[0]}Z` === "1999-12-31T17:00:00Z" ? null : `${pubShow[0]}Z`;
  newData.value.closeDate =
    `${closeShow[0]}Z` === "1999-12-31T17:00:00Z" ? null : `${closeShow[0]}Z`;
...
const submit = () => {
  ...
  if (
    Object.keys(objToSent.announcementTitle).length === 0 ||
    Object.keys(objToSent.announcementDescription).length === 0
  ) {
    objToSent.announcementTitle = null;
    objToSent.announcementDescription = null;
    newData.value = cloneNewData.value;
    alert(
      "There is an error: You have entered incorrect data(Your input data may be whitespace or an empty string)"
    );
  }
...
onMounted(async () => {
  ...
  if (typeof check === "object" || check === "new token success") {
    ...
    if (announceById.value.publishDate !== null) {
      pubDate.value = new Date(
        announceById.value.publishDate
      ).toLocaleDateString("en-CA", { dateStyle: "short" });
      pubTime.value = new Date(
        announceById.value.publishDate
      ).toLocaleTimeString("it-IT", { hour: "2-digit", minute: "2-digit" });
      clonePubDate.value = new Date(
        announceById.value.publishDate
      ).toLocaleDateString("en-CA", { dateStyle: "short" });
      clonePubTime.value = new Date(
        announceById.value.publishDate
      ).toLocaleTimeString("it-IT", { hour: "2-digit", minute: "2-digit" });
    ...
    if (announceById.value.closeDate !== null) {
      closeDate.value = new Date(
        announceById.value.closeDate
      ).toLocaleDateString("en-CA", { dateStyle: "short" });
      closeTime.value = new Date(
        announceById.value.closeDate
      ).toLocaleTimeString("it-IT", { hour: "2-digit", minute: "2-digit" });
      cloneCloseDate.value = new Date(
        announceById.value.publishDate
      ).toLocaleDateString("en-CA", { dateStyle: "short" });
      cloneCloseTime.value = new Date(
        announceById.value.publishDate
      ).toLocaleTimeString("it-IT", { hour: "2-digit", minute: "2-digit" });
      ...
      <tr class="ann-item">
        ...
        <td>
          <input
            ...
            type="text"
            maxlength="200"
            v-model.trim="newData.announcementTitle"
      ...
      <tr>
        ...
        <td>
          <input
            ...
            toolbar="full"
            theme="snow"
            v-model.trim:content="newData.announcementDescription"
            content-type="html"
      ...
      <tr>
        ...
        <td>
          <input
            ...
            v-model="newData.announcementDisplay"
            v-on:change="hasDataChanged"
          />
